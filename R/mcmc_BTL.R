...
#'
#' @return A (\code{chains}x\code{num_iters}/\code{thin})x(J+2) matrix of posterior draws, one row per posterior sample of omega, with additional columns indicating the MCMC chain index and iteration index of each posterior sample.
#'
#' @examples
...
mcmc_BTL <- function(Pi,J,a_gamma,b_gamma,nu0=NULL,num_iters=1000,groupwise=FALSE,
  ...
  mcmc <- mcmc %>% dplyr::select(chain,iteration,dplyr::everything())
  rm(counter)
