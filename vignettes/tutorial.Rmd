...
```{r}
resBTL <- mcmc_BTL(Pi=Pi,J=8,a_gamma=3,b_gamma=2,num_iters=100,chains=2,burn_prop = 0.5, thin = 1,seed = 1)
head(resBTL)
```
...
resRCBTL <- mcmc_RCBTL(Pi=Pi,J=8,a_gamma=3,b_gamma=2,lambda=5,nu0=nu0,
                       num_iters=50,nu_reps=2,chains=2,burn_prop=0.5,thin=1,seed=1)
head(resRCBTL)
```
Next, we'll view trace plots and posterior violin plots to assess convergence and mixing:
